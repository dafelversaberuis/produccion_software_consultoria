...
public class AdministrarEtapa extends ConsultarFuncionesAPI implements Serializable {
	...
	/**
	 * Obtiene los tiempos de planificaciï¿½n entre fechas
	 * 
	 * @return tiempos
	 */
	...
	/**
	 * Agrega por cada estado el total de veces que estï¿½ seleccionado
	 */
	...
	/**
	 * Determina si es vï¿½lido el diagnï¿½stico
	 * 
	 * @return ok
	 */
		...
		if (this.diagnostico != null && this.diagnostico.size() > 0) {
			...
			if (!evidenciaCompleta || !alMenosUnEstado) {
				...
				if (indicesIncompletos != null && indicesIncompletos.size() > 0) {
					...
					this.mostrarMensajeGlobalPersonalizado("Los items que estï¿½n incompletos y pendientes: " + sarta, "advertencia");
				}
	...
	public void eliminarIndicador() {
		...
		try {
			...
			// guardamos informaciï¿½n general
			...
			// guardamos la informaciï¿½n de los objetivos
			for (ObjetivoEtapaIndicador o : this.informacionGeneralIndicador.getObjetivos()) {
				...
				// guardamos la informaciï¿½n de los indicadores
			...
			// elimina info de planes de acciï¿½n
	...
	public void removerCompromiso(Compromiso aCompromiso) {
		...
		this.mostrarMensajeGlobalPersonalizado("Se removiÃ³ el compromiso del listado. Todos los cambios se verÃ¡n reflejados al guatrdar cambios", "exito");
	...
	public void asignarIndicador(Indicador aIndicador, String aVista) {
		...
		if (aVista != null && aVista.equals("ELIMINAR_INDICADOR")) {
			...
			if (ok) {
				...
				this.mostrarMensajeGlobalPersonalizado("todos los campos de informaciï¿½n general de este indicador deben estar diligenciados para poder ver/calcular los resultados", "advertencia");
			}
		}
	...
	/**
	 * Elimina un objetivo y toda su informaciï¿½n asociada
	 */
		...
		try {
			...
			for (ObjetivoEtapaIndicador o : this.informacionGeneralIndicador.getObjetivos()) {
				...
				for (Indicador i : o.getIndicadores()) {
					...
					// elimina los planes de acciï¿½n asociados a lso indicadores
					// del
					// objetivo
			...
			// elimina los planes de acciï¿½n asociados a cada indicador de este
			// objetivo
	...
	/**
	 * Agerga un nuevo plan de acciï¿½n
	 */
	...
	/**
	 * Remueve el plan de acciï¿½n seleccionado
	 * 
	...
	public void guardarFormaCompletaCompromiso() {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Compromisos actualizados (Conclusiones y/o firma). Revise diÃ¡logo", "exito");
	...
	public void guardarFormaCompletaDocumentacion() {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Documentaciï¿½n actualizada (Conclusiones y/o firma). Revise diï¿½logo", "exito");
	...
	/**
	 * Guarda de forma completa segï¿½n su vista
	 * 
	 * @param aVista
	 */
	public void guardarFormaCompleta(String aVista) {
		...
		try {
			...
			for (ObjetivoEtapaIndicador o : this.informacionGeneralIndicador.getObjetivos()) {
				...
				// guardamos la informaciï¿½n de los indicadores de todos
				for (Indicador i : o.getIndicadores()) {
					i.getCamposBD();
					conexion.actualizarBD(i.getEstructuraTabla().getTabla(), i.getEstructuraTabla().getPersistencia(), i.getEstructuraTabla().getLlavePrimaria(), null);
				}
			...
			if (aVista != null && aVista.equals("guardar")) {
				this.mostrarMensajeGlobalPersonalizado("Informaciï¿½n de indicadores actualizada (Conclusiones y firma). Revise diï¿½logo", "exito");
			}
			// vuelve y consulta lo ï¿½ltimo
			armarEstructuraIndicadores();
	...
	public void guardarFormaIncompleta() {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Informaciï¿½n de indicadores guardada, revise diï¿½logo", "exito");
	...
	public void guardarResultadosIndicador(String aTipo) {
		...
		try {
			...
			if (this.indicadorTransaccion != null) {
				...
				// eliminamos planes de acciï¿½n de bd e ingresamos los nuevos
				if (aTipo != null) {
					Map<String, Object> llaveEliminar = new HashMap<String, Object>();
					llaveEliminar.put("id_indicador", this.indicadorTransaccion.getId());
					PlanAccionIndicador plan = new PlanAccionIndicador();
	...
	public void crearIndicadorVacio(ObjetivoEtapaIndicador aObjetivoEtapaIndicador) {
		...
		try {
			...
			if (aObjetivoEtapaIndicador != null && aObjetivoEtapaIndicador.getId() != null) {
				...
				// Crea un indicador vacï¿½o y con valores por defecto
				indicadorTemp = new Indicador();
	...
	/**
	 * Cancela la eliminaciï¿½n del indicador
	 */
	...
	/**
	 * Cancela la eliminaciï¿½n del obstivo seleccionado
	 */
	...
	public void agregarCompromiso() {
		...
		this.mostrarMensajeGlobalPersonalizado("Se agregÃ³ el compromiso al listado. La informaciÃ³n quedarÃ¡ guardada al presionar guardar", "exito");
	}

	/**
	 * Crea un nuevo objetivo de forma vacï¿½a
	 * 
	...
	public void crearObjetivoVacio(String aPrimeraTransaccion) {
		...
		try {
			...
			if (aPrimeraTransaccion != null && aPrimeraTransaccion.equals(IConstantes.AFIRMACION)) {
				// crea la informaciï¿½n general de indicadores
				...
				// guarda la informaciï¿½n que lleve en el momento para que no se
				// pierda
				this.informacionGeneralIndicador.getCamposBD();
				conexion.actualizarBD(this.informacionGeneralIndicador.getEstructuraTabla().getTabla(), this.informacionGeneralIndicador.getEstructuraTabla().getPersistencia(), this.informacionGeneralIndicador.getEstructuraTabla().getLlavePrimaria(), null);
			...
			// crea un registro vacï¿½o de objetivo
			...
			// a su vez le crea un indicador vacï¿½o y con valores por defecto
			indicadorTemp = new Indicador();
			...
			// Vuelve y lo llama para recargar con lo ï¿½ltimo
			armarEstructuraIndicadores();
	...
	/**
	 * Selecciona la actividad de docuemntaciï¿½n en la consulta
	 */
	...
	/**
	 * Selecciona la opciï¿½n de documentaciï¿½n
	 */
	...
	public void onRowSelect(SelectEvent event) {
		...
		this.mostrarMensajeGlobalPersonalizado("Actividad seleccionada:" + this.planificacionSeleccionada.getTareaProyecto().getTarea() + ". Espere un momento se esta abriendo diï¿½logo con opciones...", "advertencia");
	...
	/**
	 * Detremina si una planificaciï¿½n estï¿½ o no registrada
	 * 
	 * @return true:false
	 */
	...
	/**
	 * Determina si un diagnï¿½stico estï¿½ o no incompleto
	 * 
	 * @return ok
	 */
	...
	public void imprimirEstadosTareas() {
		try {
			...
			// para que no envï¿½e correo siempre
	...
	/**
	 * Determina si existe ya un diagnï¿½stico previo guardado
	 * 
	 * @return existente
	 */
	...
	public void eliminarCompromiso() {
		...
		try {
			...
			conexion.commitBD();
			this.mostrarMensajeGlobalPersonalizado("Compromisos eliminados con Ã©xito. Puede comenzar de nuevo!", "exito");
	...
	public void eliminarDocumentacion() {
		...
		try {
			...
			// Elimina la documentaciï¿½n guardada
			...
			// elimina la documentaciï¿½n general
			...
			conexion.commitBD();
			this.mostrarMensajeGlobalPersonalizado("Documentaciï¿½n eliminada con ï¿½xito para el proyecto/cliente", "exito");
	...
	public boolean isValidacionesDocumentacion(TareaProyecto aTareaProyecto) {
		...
		try {
			...
			// Aquï¿½ se valida tanto los documentos generales como los del
			// consultor
	...
	/**
	 * Verfica que posea documentaciï¿½n
	 * 
	 * @return ok
	 */
	...
	/**
	 * Actualiza la tarea planificada, envï¿½a correos si es necesario
	 */
	...
	/**
	 * Decide que transacciï¿½n realizar
	 * 
	 * @param aDecision
	 */
	...
	public void aprobarPlanificacion() {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Planificaciï¿½n aprobada! Ya puede comenzar a ejecutarse", "exito");
				...
				for (Compromiso p : this.compromisos) {
					...
					if (p.getFechaSeguimiento() != null && p.getFechaParaCuando() != null) {
						if (!(formato.format(p.getFechaParaCuando()).compareTo(formato.format(p.getFechaSeguimiento())) >= 0)) {
							ok = false;
							this.mostrarMensajeGlobalPersonalizado("Las fechas para cuando del seguimiento deben ser mayor o igual a la fechas de seguimiento si Ã©stas se diligencian", "advertencia");
							break;
						}
					}
					...
					if (p.getFechaSeguimiento() != null || (p.getEstado() != null && !p.getEstado().trim().equals("")) || (p.getObservacionesEstado() != null && !p.getObservacionesEstado().trim().equals("")) || p.getFechaParaCuando() != null) {
						...
						if (!(p.getFechaSeguimiento() != null && (p.getEstado() != null && !p.getEstado().trim().equals("")) && (p.getObservacionesEstado() != null && !p.getObservacionesEstado().trim().equals("")))) {
							ok = false;
							this.mostrarMensajeGlobalPersonalizado("Recuerde que si diligenciÃ³ para una fila espeÃ­fica algÃºn campo con (**) Ã©sta fila debe diligenciar todos sus campos con (**)", "advertencia");
							break;
						}
	...
	public void actualizarDocumentacion() {
		...
		try {
			...
			if (ok) {
				...
				this.mostrarMensajeGlobalPersonalizado("Documentaciï¿½n guardada exitosamente", "exito");
	...
	public void actualizarCronograma() {
		...
		try {
			...
			if (ok) {
				...
				this.mostrarMensajeGlobalPersonalizado("Revise la informaciï¿½n de la planificaciï¿½n/cronograma detallado. Si se diligenciï¿½, las fechas finales deben ser mayores o iguales a las iniciales y las fechas de seguimiento dentro del rango inicial-final", "advertencia");
	...
	/**
	 * Actualiza la planificaciï¿½n siendo inserciï¿½n o actualizaciï¿½n
	 * 
	 */
	public void actualizarPlanificacion() {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Planificaciï¿½n guardada exitosamente", "exito");
			this.abrirModal("panelResumen");
	...
	/**
	 * Actualiza el diagnï¿½stico luego de validado
	 */
	public void actualizarDiagnostico(String aForma) {
		...
		try {
			...
			if (aForma.equals(IConstantes.FORMA_COMPLETA)) {
				...
				// Ya el correo no es automï¿½tico, mï¿½s bien el consultor decide
				// decide
				// cuando enviarlo
	...
	/**
	 * Actualiza de una vez el diagnï¿½stico o advierte
	 */
	...
	/**
	 * Valida que si selecionï¿½ no aplica como estado deselecciona los demï¿½s.
	 * Cambiï¿½ a solo seleccionar el que es
	 * 
	 * @param aEstados
	 * @param aEstado
	 */
	...
	/**
	 * Guarda la planificaciï¿½n
	 */
	public void enviarCorrero() {

		this.mostrarMensajeGlobalPersonalizado("Se ha enviado un correo informando sobre aprobaciï¿½n del plan de trabajo", "exito");
	...
	/**
	 * Determina si todos los periodos de un indicador estï¿½n nulos
	 * 
	 * @param aIndicador
	 * @return nulos
	 */
	...
	/**
	 * Determina si se debe o no tomar plan de acciï¿½n
	 * 
	 * @return tomar
	 */
	...
	/**
	 * Obtiene el color del texto segï¿½n frecuencia elegida y el periodo
	 * 
	 * @return color
	 */
	...
	/**
	 * Obtiene el color de la celda segï¿½n frecuencia elegida y el periodo
	 * 
	 * @return color
	 */
	...
	/**
	 * Arma la estructura de la planificaciï¿½n
	 */
	...
	/**
	 * Consulta todos los datos para la etapa de planificaciï¿½n
	 */
	...
	/**
	 * Consulta los datos completos de un proyecto de consultorï¿½a de un cliente
	 * 
	...
	/**
	 * Obtiene el registro de planificaciï¿½n
	 * 
	...
	/**
	 * Establece el registro de planificaciï¿½n
	 * 
	...
	/**
	 * Obtiene informaciï¿½n general de un indicador
	 * 
	...
	/**
	 * Obtiene planificaciï¿½n general
	 * 
	 * @return registroPlanificacion
	 */
	...
	/**
	 * Establece planificaciï¿½n general
	 * 
	...
	// listas
	/**
	 * Obtiene el listado de clientes que puede manipular el diagnï¿½stico
	 * 
	...
	/**
	 * Obtiene un listado de planificaciï¿½n
	 * 
	...
	/**
	 * Establece un listado de planificaciï¿½n
	 * 
	 * @param planificacion
	 */
	...
	/**
	 * Obtiene los datos actuales del grï¿½fico
	 * 
	...
	public void recibirArchivo(FileUploadEvent event) {
		...
		try {
			...
			this.mostrarMensajeGlobalPersonalizado("Archivo recibido, guÃ¡rdelo.", "advertencia");
		} catch (Exception e) {
			IConstantes.log.error(e, e);
		}
	...
	/**
	 * Cancela la creaciï¿½n de un documento diagnostico
	 */
